...
import { MAPS, MAJOR_GODS, PANTHEONS, MCL_ROUND_MAPS, CASCA_GROSSA_GROUP_POOL, CASCA_GROSSA_PLAYOFF_POOL } from '../../constants';
...
export function LobbyCreation({
  ...
  const enableAllPantheons = () => {
    setConfig((prev: any) => ({ 
      ...prev, 
      allowedPantheons: MAJOR_GODS.filter(g => g.culture !== 'Aztec').map(g => g.id), 
      preset: 'CUSTOM' 
    }));
  };
  ...
  const togglePantheonGroup = (culture: string) => {
    ...
    setConfig((prev: any) => {
      ...
      return { ...prev, allowedPantheons: newAllowed, preset: 'CUSTOM' };
    });
  };
      ...
      <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {[1, 2, 3, 4, 5, 6, 7].map(round => (
                        <button
                          key={round}
                          onClick={() => setConfig((prev: any) => ({ ...prev, mclRound: round }))}
                          className={cn(
                            "py-2 rounded-lg border text-sm font-bold transition-all",
        ...
        <div>
          ...
          <div className="grid grid-cols-6 gap-2 mb-3">
            {['BO1', 'BO3', 'BO5', 'BO7', 'BO9', 'CUSTOM'].filter(type => {
              if (config.preset === 'CASCA') {
                if (config.tournamentStage === 'GROUP') return type === 'BO1';
                return ['BO3', 'BO5'].includes(type);
              }
              return true;
            }).map(type => (
              <button
        ...
        <div className="space-y-2">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-slate-400 uppercase tracking-widest">
                {t.manualPantheons} ({config.allowedPantheons.length})
              </span>
            </div>
            <div className="flex items-center gap-2">
                        ...
                        <div key={culture} className="space-y-3">
                          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                            <span className="text-xs font-black text-slate-500 uppercase tracking-widest">
                              {t.pantheons?.[culture] || culture} ({selectedInPantheon.length}/{godsInPantheon.length})
                            </span>
                            {!isLocked('allowedPantheons') && culture !== 'Aztec' && (
                              <button
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {godsInPantheon.map(god => {
                              ...
                              const isGodLocked = isLocked('allowedPantheons') || isAztec;
                              return (
                                <button
                                  key={god.id}
                                  disabled={isGodLocked}
                                  ...
                                  className={cn(
                                    ...
                                    config.allowedPantheons.includes(god.id)
                                      ? "bg-amber-500/10 border-amber-500/50"
                                      : "bg-slate-900/50 border-slate-800 hover:border-slate-700",
                                    isGodLocked && "cursor-not-allowed"
                                  )}
                                >
                                  <div className="w-14 h-14 rounded-md overflow-hidden bg-slate-800 flex-shrink-0">
                                  ...
                                  {isGodLocked && (
                                    <div className="absolute inset-0 flex items-center justify-center bg-slate-950/10 backdrop-blur-[0.5px]">
                                      <Lock className="w-4 h-4 text-amber-500/50 shadow-lg" />
                                    </div>
                                  )}
                                </button>
                              );
