...
export function DraftUI(props: DraftUIProps) {
  const { lobby, isCaptain1, isCaptain2, handleAction, handlePickerAction, t, setLobbyId, onHome, error, setError, getShareableUrl, updateRoster, clearSubs, showBugModal, setShowBugModal } = props;
  ...
  // Ensure showSummary is updated if the lobby finishes while watching
  useEffect(() => {
    if (props.isSpectator && (lobby.status === 'finished' || lobby.phase === 'reporting')) {
      // Only auto-show summary if we weren't already in a specific view
        ...
        {props.isSpectator && (
          <div className="px-6 py-2 bg-slate-900/50 border-b border-slate-800 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  const url = `${window.location.origin}/overlay/${lobby.id}`;
