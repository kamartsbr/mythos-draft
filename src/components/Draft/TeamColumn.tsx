...
export function TeamColumn({ team, lobby, isCurrentTurn, t, isCaptain1, isCaptain2 }: TeamColumnProps) {
  ...
  const teamPicks = lobby.picks
    .filter(p => p.team === team)
    .sort((a, b) => {
      // Visual order:
      // Team A: 1 (Red) -> Top, 5 (Yellow) -> Middle, 4 (Orange) -> Bottom
      // Team B: 2 (Pink) -> Top, 6 (Cyan) -> Middle, 3 (Blue) -> Bottom
      const orderA = { 1: 0, 5: 1, 4: 2 };
      const orderB = { 2: 0, 6: 1, 3: 2 };
      
      if (team === 'A') {
        return (orderA[a.playerId as keyof typeof orderA] ?? 99) - (orderA[b.playerId as keyof typeof orderA] ?? 99);
      } else {
        return (orderB[a.playerId as keyof typeof orderB] ?? 99) - (orderB[b.playerId as keyof typeof orderB] ?? 99);
      }
    });
  const teamGodBans = lobby.replayLog
    .filter(step => step.action === 'BAN' && step.target === 'GOD' && step.player === team && step.gameNumber === lobby.currentGame);

  const teamMapBans = lobby.replayLog
    .filter(step => step.action === 'BAN' && step.target === 'MAP' && step.player === team && step.gameNumber === lobby.currentGame);
  
  const teamMapPicks = lobby.replayLog
        ...
        {lobby.config.teamSize === 1 ? (
          <div className="flex flex-wrap gap-3">
            {teamPicks.map((pick, idx) => {
              ...
              const currentlyPickingPlayerId = originalTeamPicks[godPicksBeforeThisTurn]?.playerId;
              
              const isGodTurn = turn?.target === 'GOD';
              const isMyTeamTurn = turn?.player === team || turn?.player === 'BOTH';
              const isCurrentPlayerTurn = isGodTurn && isMyTeamTurn && pick.playerId === currentlyPickingPlayerId;
