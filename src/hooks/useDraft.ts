...
export function useDraft(
  ...
  useEffect(() => {
    ...
    if (lobby.phase !== 'waiting' && lobby.phase !== 'ready' && lobby.phase !== 'finished') {
      if (optimisticReady !== null) setOptimisticReady(null);
      return;
    }

    const currentReady = (lobby.phase === 'ready' || lobby.phase === 'finished')
  ...
  const handleAction = useCallback(async (actionIdArg: any, playerId?: number, playerName?: string, options?: { isRandom?: boolean }) => {
    ...
    // Ensure actionId is a string. If it's an event or something else, ignore it.
