...
import { StreamerOverlay } from './components/Draft/StreamerOverlay';
...
function AppContent() {
  ...
  // Simple Routing for Streamer Overlay
  const isOverlay = window.location.pathname.startsWith('/overlay/');
  const overlayLobbyId = isOverlay ? window.location.pathname.split('/')[2] : null;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-amber-500/30 relative flex flex-col">
      {isOverlay && overlayLobbyId ? (
        <StreamerOverlay lobbyId={overlayLobbyId} />
      ) : (
        <>
              ...
              <div className="absolute top-[750px] bottom-[450px] left-0 w-[18%] flex flex-col opacity-60 pointer-events-none hidden xl:flex z-0 overflow-hidden">
                ...
                {[
                  ...
                  <div key={i} className="relative h-[450px] w-full shrink-0">
                    <img src={url} alt="" className="w-full h-full object-cover filter brightness-125 saturate-150 contrast-110" referrerPolicy="no-referrer" />
