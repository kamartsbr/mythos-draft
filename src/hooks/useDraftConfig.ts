...
import { MAPS, MAJOR_GODS, RANKED_MAP_POOL, CASCA_GROSSA_POOL, CASCA_GROSSA_GROUP_POOL, CASCA_GROSSA_PLAYOFF_POOL, MCL_ROUND_MAPS, MCL_MAP_POOL } from '../constants';
      ...
      return parsed.map((preset: LobbyConfig) => ({
        ...
        allowedPantheons: Array.isArray(preset.allowedPantheons)
          ? preset.allowedPantheons.filter(id => {
              ...
              return god && god.culture !== 'Aztec';
            })
          : MAJOR_GODS.filter(g => g.culture !== 'Aztec').map(g => g.id)
      }));
    } catch (e) {
  ...
  const applyPreset = useCallback((preset: string) => {
    setConfig(prev => {
      ...
      if (preset === 'CUSTOM') {
        ...
        newConfig.allowedPantheons = MAJOR_GODS.filter(g => g.culture !== 'Aztec').map(g => g.id);
        newConfig.mapTurnOrder = [];
        newConfig.godTurnOrder = [];
      } else if (preset === 'CASCA') {
        newConfig.allowedPantheons = MAJOR_GODS.filter(g => g.culture !== 'Aztec').map(g => g.id);
        newConfig.teamSize = 1;
        newConfig.isExclusive = true;
        newConfig.pickType = 'alternated';
        newConfig.mapBanCount = 2;
        newConfig.banCount = 2;
        newConfig.acePick = false;
        newConfig.tournamentStage = prev.tournamentStage || 'PLAYOFFS';
        newConfig.mapTurnOrder = [];
        newConfig.godTurnOrder = [];
        
        if (newConfig.tournamentStage === 'GROUP') {
          ...
          newConfig.loserPicksNextMap = true;
        }
      } else if (preset === 'MCL') {
        newConfig.allowedMaps = MCL_MAP_POOL;
        newConfig.allowedPantheons = MAJOR_GODS.filter(g => g.culture !== 'Aztec').map(g => g.id);
        ...
        newConfig.mclRound = 1;
        newConfig.tournamentStage = 'GROUP';
        newConfig.mapTurnOrder = [];
        const roundMap = MCL_ROUND_MAPS[1];
