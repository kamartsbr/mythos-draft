...
export function DraftBoard(props: DraftBoardProps) {
  const { lobby, isCaptain1, isCaptain2, t, handleReady, isProcessing, optimisticReady, reportScore, resetVotes, copyUrl, handlePickerAction, onHome, viewGameIndex, setViewGameIndex, error, setError, isAdmin, forceReset, resetCurrentGame, forceFinish, forceUnpause, updateRoster, clearSubs } = props;
  ...
  const readyA = (isCaptain1 && optimisticReady !== null) ? optimisticReady : lobby.readyA;
  const readyB = (isCaptain2 && optimisticReady !== null) ? optimisticReady : lobby.readyB;
            ...
            <div className="flex flex-col items-center gap-6">
              ...
              <div className="flex gap-8">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <div className={cn(
                    "px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest",
                    lobby.readyA ? "bg-green-500/20 text-green-500" : "bg-slate-800 text-slate-600"
                  )}>
                    {readyA ? t.ready : t.waiting}
                  </div>
                </div>
                <div className="h-8 w-px bg-slate-800" />
                <div className="flex flex-col items-center gap-2">
                  ...
                  <div className={cn(
                    "px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest",
                    readyB ? "bg-green-500/20 text-green-500" : "bg-slate-800 text-slate-600"
                  )}>
                    {readyB ? t.ready : t.waiting}
                  </div>
                </div>
              </div>
            </div>
