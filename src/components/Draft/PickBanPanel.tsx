...
import { Search, Info, Check, X, Shield, Sword, Map as MapIcon, Lock, Users, Clock, Dices, RefreshCw } from 'lucide-react';
...
export function PickBanPanel({ 
  ...
  optimisticAction, isMyTurn, myTeam
}: PickBanPanelProps) {
  ...
  useEffect(() => {
    if (lobby.phase !== lastPhase && lobby.status === 'drafting') {
      ...
      // We no longer show the 800ms blocking overlay as it causes perceived delay
  ...
  const modalsPortal = typeof document !== 'undefined' ? createPortal(
    <>
      <ConfirmModal 
        isOpen={showRequestResetConfirm}
        onClose={() => setShowRequestResetConfirm(false)}
        onConfirm={() => {
          requestReset();
          setShowRequestResetConfirm(false);
        }}
        title={t.requestResetTitle || "REQUEST GAME RESET"}
        message={t.requestResetMsg || "This will pause the draft and ask your opponent for a reset of the current game only. Series progress will be saved. Do you want to proceed?"}
        confirmText={t.requestResetBtn || "REQUEST RESET"}
      />
      <ConfirmModal 
            ...
            <div className="flex flex-col items-center gap-4 w-full">
              ...
              {(isCaptain1 || isCaptain2) && (
                <button
                  ...
                  className="flex flex-row items-center justify-center gap-2 px-6 py-3 mt-4 bg-red-600/10 hover:bg-red-600/20 text-red-500 border border-red-600/30 rounded-2xl transition-all font-bold text-xs uppercase tracking-widest w-full max-w-sm"
                >
          ...
          <div className="w-full flex items-center justify-between">
            <div className="flex items-center gap-4">
              {(isCaptain1 || isCaptain2) && (
                ...
                >
                  <RefreshCw className="w-3 h-3 mr-2 inline-block" />
                  {t.resetGameDraft || "RESET GAME DRAFT"}
                </button>
              )}
            </div>
